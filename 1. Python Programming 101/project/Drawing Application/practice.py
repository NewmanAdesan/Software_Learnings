...
graphics_command = GraphicCommands.PyList()


def main():
	# global graphics_command
	...
	side_bar = tkinter.Frame(root, padx=5, pady=5)
	...
	def undo_handler():
		graphics_command.remove_last()
		the_turtle.clear()
		the_turtle.penup()
		the_turtle.goto(0, 0)
		the_turtle.pendown()

		for cmd in graphics_command:
			cmd.draw(the_turtle)
	...
	"""
		- create a menu widget object (outer layer)
		- add it to the root window
		- create another menu widget object (inner layer), place inside the outer layer menu object
		- add options and their functionality to the inner layer menu
			* "New"
				- This pretty much creates a new canvas.
				  clearing turtle drawing, 
				  setting turtle to initial state,
				  clearing graphics command memory


			* "Load..."
				- This collects an XML file depicting drawing commands
				  then converts all the drawing command in the xml file into a Command Structure
				  places the Command Structures into a list "graphics commands"
				  from the list, draws each command


			* "Load Into..."
				- This collects an XML file depicting drawing commands
				  then parses the xml file of drawing commands into a "graphics commands" list
				  like we did in the "Load..." option.

				- what is different here is that
				  we are going to add the "graphics commands" list gotten from the xml file
				  to the current "graphics commands" list
				  then draw them all together.


			* "Save As..."
				- This convert the graphics command list into an XML file
				  by writing each command in the graphics command list into a file 
				  whose filename is specified by user

			* "Exit"
	"""

	# Setup inner and outer layer menu widget object
	outer_menu = tkinter.Menu()
	root.configure(menu=outer_menu)
	...
	def load_option():
		...
		graphics_command = parse(filename)

		# draw the commands in the loaded file
		for command in graphics_command:
	...
	def load_into_option():
		...
		file_graphics_command = parse(filename)
		...
		for command in file_graphics_command:
			graphics_command.append(command)
